...
export default function BlogDeleteButton({ articleId, articleTitle }: { articleId: string; articleTitle: string }) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await adminFetch(`/api/admin/blog/${articleId}`, { method: "DELETE" });
